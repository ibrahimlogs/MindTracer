...
The evaluator can return `confirmed`, `probable`, `uncertain`, `not_detected`, or `arithmetic_only`. It records supported, weakened, and rejected hypothesis IDs, recommends an intervention family, and sets a starting level from 1 to 4. Step 8 now uses that verified state to select and animate the bounded intervention.
